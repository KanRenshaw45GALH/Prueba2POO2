...
public class CompartirElementoControlador {
    ...
    @FXML private TextField TextEmailDestino;
    ...
    private void compartir(ActionEvent event) {
        ...
        String emailDestino = TextEmailDestino.getText().trim();
        ...
        if (emailDestino.isBlank()) {
            mostrarError("Ingresa el email del usuario destino.");
            return;
        }
        ...
        Usuario usuarioDestino = listadoUsuarios.buscarPorEmail(emailDestino);

        if (usuarioDestino == null) {
            mostrarError("No se encontró un usuario con ese email.");
            return;
        }

        if (usuarioDestino.getEmail().equalsIgnoreCase(usuarioActivo.getEmail())) {
